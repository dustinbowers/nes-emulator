...
use registers::status_register::PpuStatusRegister;
...
pub struct PPU {
    ...
    pub status_register: PpuStatusRegister, // $2002 (R)
...
impl PPU {
    ...
    pub fn reset(&mut self) {
        ...
        self.status_register = PpuStatusRegister::new();
    ...
    pub fn exec_operation(&mut self, operation: PpuOperation) {
        match operation {
            ...
            PpuOperation::ClearVBlank => {
                trace_ppu_event!(
                    "VBLANK CLEAR vblank={} frame={} scanline={} dot={} ppu_cycle={} ticks_since_vblank_start={}",
                    self.status_register.vblank_active(),
                    self.frame_is_odd as u8,
                    self.scanline,
                    self.cycles,
...
impl PPU {
    ...
    pub fn run_until_vblank(&mut self) {
        ...
        while !self
            .status_register
            .contains(PpuStatusRegister::VBLANK_STARTED)
        {
            self.tick();
        }
    }
...
impl PPU {
    fn render_dot(&mut self) -> u8 {
        ...
        if show_bg
            ...
            && !self
                .status_register
                .contains(PpuStatusRegister::SPRITE_ZERO_HIT)
        {
